...
public class Message {
   ...
   * @param length The length of this message, which specifies how many SubMessages are contained in
   *          this message.
  ...
  public float[] translateData(byte[] data, byte[] time, int numColumns) {
    /*
     * The time byte array is four bytes long. The timestamp is generated by concatenating the third
     * and second bytes, adding that to the concatenation of the first and zeroth bytes divided by
     * 0x8000, then subtracting 1.
     */
    ...
    /*
     * Values which are not known to this message (all other columns in the application's main data
     * array), are set to Float.MAX_VALUE during the translation phase to signal to the
     * normalization function that they should not be regarded as data sent in a CAN message.
     */
     ...
     * CAN bus into the intended values. The messageLength ensures that we don't go out of the range
     * of the data[] array. Two consecutive bytes from the data[] are sent to each SubMessage.
     */
